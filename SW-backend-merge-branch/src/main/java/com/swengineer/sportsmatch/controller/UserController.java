...
import com.swengineer.sportsmatch.dto.UserDeleteRequestDTO;
...
public class UserController {
    ...
    public ResponseEntity<Void> deleteUser(
            ...
            @RequestBody UserDeleteRequestDTO request) { // 탈퇴 요청 시 사용자가 입력한 비밀번호를 받음.
        boolean isPasswordValid = userService.validatePassword(userId, request.getPassword());
        if (!isPasswordValid) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        } // 비밀번호 확인.
        userService.deleteUser(userId, request.getPassword()); //해당 userID 삭제.
